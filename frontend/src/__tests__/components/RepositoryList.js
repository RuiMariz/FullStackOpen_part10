...
describe('RepositoryList', () => {
  describe('RepositoryListContainer', () => {
    it('renders repository information correctly', () => {
      ...
      expect(getAllByTestId('forks')[0]).toHaveTextContent(convertToThousands(repositories.edges[0].node.forksCount));
      expect(getAllByTestId('stars')[0]).toHaveTextContent(convertToThousands(repositories.edges[0].node.stargazersCount));
      expect(getAllByTestId('rating')[0]).toHaveTextContent(repositories.edges[0].node.ratingAverage);
      expect(getAllByTestId('reviews')[0]).toHaveTextContent(convertToThousands(repositories.edges[0].node.reviewCount));
      ...
      expect(getAllByTestId('forks')[1]).toHaveTextContent(convertToThousands(repositories.edges[1].node.forksCount));
      expect(getAllByTestId('stars')[1]).toHaveTextContent(convertToThousands(repositories.edges[1].node.stargazersCount));
      expect(getAllByTestId('rating')[1]).toHaveTextContent(repositories.edges[1].node.ratingAverage);
      expect(getAllByTestId('reviews')[1]).toHaveTextContent(convertToThousands(repositories.edges[1].node.reviewCount));
